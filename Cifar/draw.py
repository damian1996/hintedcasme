...
def train_or_eval(data_loader, data_loader_w_norm, spoiled_loader_w_norm, classifier, decoder, args):
    classifier_criterion = nn.CrossEntropyLoss().to(device)
    decoder.eval()
    classifier.eval()
    
    for i, (input, target) in enumerate(data_loader_w_norm):
        input, target = input.to(device), target.to(device)
        batch, expected_output = input, target
        break

    for i, (input, target) in enumerate(data_loader_w_norm):
        hinted_input, h_target = input.to(device), target.to(device)
        h_batch, h_output = hinted_input, h_target
        break

    original = batch[0]
    hinted = hinted_input[0]    
    with torch.set_grad_enabled(False):
        output, layers = classifier(h_batch)
        ...
        with torch.set_grad_enabled(False):
            ...
            bin_input_m = h_batch * (1-binarized_mask)
            bin_output_m, _ = classifier(bin_input_m)
            
            masked = bin_input_m[0]
            print(masked.shape)
    
    import cv2
    original = original * 256
    original = torch.cuda.LongTensor(original)
    #map(original, lambda=x: int(x))
    print(original)
    cv2.imwrite('./original.png', original.cpu().detach().numpy().transpose((1,2,0)))
    cv2.imwrite('./hinted.png', hinted.cpu().detach().numpy().transpose((1,2,0)))
    cv2.imwrite('./masked.png', masked.cpu().detach().numpy().transpose((1,2,0)))
    
    '''
    fig=plt.figure(figsize=(8, 8))
    columns = 3
    rows = 2
    images = [original, original, hinted, masked, original, hinted, masked]
...
def restore(args, classifier, decoder, optimizer):
    filename = 'BNOriginalMaskerrandom.chk'
...
def main(prob=0.):
    ...
    filename = 'BNOriginalMaskerrandom.chk'
    last_checkpoint_file = os.path.join(SAVED_MODELS, filename)
    starting_epoch = load_checkpoint(optimizer, classifier, decoder, last_checkpoint_file)

    #restore(args, classifier, decoder, optimizer)
    train_loader, val_hint_loader, val_hint_loader_w_norm, val_pure_loader, val_pure_loader_w_norm = data_loading(args)
    
    #train_or_eval(val_hint_loader, val_hint_loader_w_norm, classifier, decoder, args)
    train_or_eval(val_pure_loader, val_pure_loader_w_norm, val_hint_loader_w_norm, classifier, decoder, args)

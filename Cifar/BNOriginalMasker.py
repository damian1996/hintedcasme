...
def train_or_eval(data_loader, data_loader_w_norm, classifier, decoder, args, to_draw, version, train=False, optimizer=None, epoch=None):
    ...
    for i, (input, target) in enumerate(data_loader):
        ...
        with torch.set_grad_enabled(train):
            ...
            # mask = torch.zeros_like(input)            # mask = torch.zeros_like(input)
            # mask[:, 0, :4, 0] = 1
            # mask[:, 0, :4, 0] = 1
            mask = decoder(layers)
            mask = mask.to(device)
        ...
        if train:
            ...
            # casme_loss = args.lambda_r * F.relu(mask_mean).mean()
...
def restore(args, classifier, decoder, optimizer, to_draw):
    filename = 'OriginalCifarrandom.chk'
...
if __name__ == '__main__':
    logpath = sys.argv[1]
    print(logpath)
    main(logpath, lambda_r=10, epochs=460, prob=1.)

...
def main(logpath, lambda_r=10, epochs=300, prob=0.):
    ...
    _, val_hint_loader, val_hint_loader_w_norm, val_pure_loader, val_pure_loader_w_norm = dataloaders_preparation(args)
    best_acc, best_acc_pure, best_acc_mask = 0., 0., 0.
    
    for epoch in range(starting_epoch, args.epochs):
        print(f"Epoch {epoch}. Results {best_acc} {best_acc_pure} {best_acc_mask}")
        
        silly_leaning_rate_scheduler(optimizer, 'classifier', epoch, args)
        ...
        print("After epoch", float(val_hint['acc'], float(val_pure['acc']), float(val_pure['acc_m']))
...
if __name__ == '__main__':
    logpath = sys.argv[1]
    main(logpath, lambda_r=10, epochs=90, prob=0.)